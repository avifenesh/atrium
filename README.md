...
One of them is reusable on its own: **[revuto](https://github.com/avifenesh/revuto)** — a
local, supplier-agnostic autonomous PR reviewer that learns from maintainer feedback. If
you run it, clone it and point `config.revuto.snapshotUrl` at its local snapshot endpoint;
the `revuto` collector then surfaces its state in atrium. Otherwise leave that collector
disabled.
