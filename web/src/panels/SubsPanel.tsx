...
function ResetsIn({ iso }: { iso: string | null }) {
  ...
  return (
    <span className="font-mono text-[10px] text-mist-faint" title={new Date(iso).toLocaleString()}>
      ...
      {services.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {services.map((s, i) => (
            <article
              key={s.id}
              className="glass rise flex flex-col p-4"
              style={{ '--rise-i': i } as CSSProperties}
            >
              <header className="flex items-center justify-between gap-2">
                <h3 className="truncate font-display text-lg italic text-mist">{s.name}</h3>
              ...
              {s.detail &&
                (s.status === 'not-connected' ? (
                  ...
                  <div className="mt-1 text-xs text-mist-dim">{s.detail}</div>
                ))}
                    ...
                    return (
                      <div key={u.label}>
                        <div className="flex items-baseline justify-between gap-2">
                          <span className="truncate text-[11px] text-mist-dim">{u.label}</span>
                          <span className="shrink-0 font-mono text-[11px] text-mist-dim">{Math.round(u.usedPct)}%</span>
                        </div>
